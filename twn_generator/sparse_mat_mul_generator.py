...
def set_outputs( depths, output_idxs, BW, no_inputs ):
    ...
    for idx, oi in enumerate( output_idxs ):
        ...
        if delay_reg == 0 or get_name( oi, no_inputs ) == '0':
            ...
            computations += [ "out_" + str(idx) + " <= " + get_name( oi, no_inputs ) + ";" ]
        else:
            declarations += [ "reg [" + str(delay_reg-1) + ":0][" + str(BW-1) + ":0] out_" + str(idx) + ";" ]
            declarations += [ "assign out[" + str(idx) + "] = out_" + str(idx) + "[" + str(delay_reg - 1) + "];" ]
            computations += [ "out_" + str(idx) + " <= { out_" + str(idx) + "[" + str( delay_reg - 2 ) + ":0], " + get_name( oi, no_inputs ) + "};" ]
    output_txt = "\n".join( declarations )
...
"""
    ...
    for op in ops:
        ...
        module_body += create_op( names, op[4], op[5:], BW_in, BW_out, "op_" + str(op[0]), stage_reset, depths[op[1]] )
        if op[2] != -1:
    ...
    module_body += set_outputs( depths, output_idxs, BW_out, no_inputs )
